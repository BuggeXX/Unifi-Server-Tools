    ...
    foreach ($Site in $UData.Sites) {
        foreach ($Device in ($Site.Devices.data) | Where-Object -Property upgradable -like 'true') {
            ...
            if ($DeviceStatus.state -like 1) {
                Write-Warning -Message "Starting Upgrade for $($Device.name) on site $($Site.SiteName)"
                ...
                $TimeOut = 0
                while ($DeviceStatus.state -notlike 1) {
                    Start-Sleep 30
...
Function Invoke-UAutoInform {
    #javaw -jar ubnt-discovery-v2.5.1.jar
}
